...
% *****************************************
% debugSamplingPattern(Lin, Par, Set, opt);
% *****************************************
...
opt.fs = 1/opt.TR;   
opt.timeFull = [0:1:size(SG,3)-1] * opt.TR;
...
opt.timestamp = opt.timestamp(:,1:size(SG,3)*opt.sgI);
...
opt.timestamp = opt.timestamp(:,1+opt.sgI*opt.rampSS:end);
...
if ~isinf(opt.recTime)
    ...
    opt.timestamp = opt.timestamp(:,(firstSample*opt.sgI)-(opt.sgI-1):lastSample*opt.sgI);
end      
