function [data, samp, param, cMaps, rawData, ref] = readWrapper3(p)
% =========================================================================
% Rizwan Ahmad (ahmad.46@osu.edu)
% A wrapper for reading Siemens data
% Last Modified: 12/12/2014
%
% Added compatibility with interleaved self-gating signals
% User defined flow
% New user parameter: sgI    = self-gating interval
%                     flow   = 0 or 1
%
% Modified 11/21/2018 by Aaron Pruitt
% =========================================================================
...
if isfield(p,'nAmp'),    nAmp     = p.nAmp;       else, nAmp    = 1;            end % noise amplifiation fator
...
if isfield(p,'sgI'),     sgI      = p.sgI;        else, sgI     = 0;            end % self-gating interval: 0 means no self-gating
if isfield(p,'flow'),    flow     = p.flow;       else, flow    = 1;            end % self-gating interval: 0 means no self-gating
if isfield(p,'fName'),   fName    = p.fName;      else, error('file name missing');  end  % file name

%% VB or VD data file from Siemens Scanner
rawData = mapVBVD(fName,'ignoreSeg'); % Ignore segment here to save memory
disp('---------------- Data dimensions ---------------- ');
if length(rawData) > 1
    rawData = rawData{2};
end
disp(rawData.image.sqzDims);

% Check if self-gating is used or not
if sgI
    data = rawData.image.unsorted;
else
    data = rawData.image{''};
end

% Append the reference scan to the actual scan
if isfield(rawData, 'refscan')
    warning('Reference scan detected. The code is not equipped to distinguish between intergrated and separate reference scan');
     ref = rawData.refscan{''};
%     tmp = rawData.refscan{''};      
%     tmpN = (size(data,3)-size(tmp,3))/2;
%     data = cat(4, data, padarray(tmp, [0 0 tmpN,0], 0, 'both'));
else
    ref = [];
end

% Read noise if present
if isfield(rawData, 'noise')
    nScan = rawData.noise(); 
else
    nScan =[]; 
end
...
param.dim           = rawData.image.sqzDims;                                        % Matrix dimension
param.slPos         = unique((rawData.image.slicePos)', 'rows','stable')';          % Slice posiition
param.slThck        = rawData.hdr.MeasYaps.sSliceArray.asSlice{1}.dThickness;       % Slice thickness
param.FOV           = [(rawData.hdr.Dicom.flReadoutOSFactor*rawData.hdr. ...        % FOV in mm ??? is oversampling always 2
                      Config.ReadFoV)*(1-sum(fd)), rawData.hdr.Config.PhaseFoV]; 
param.matSz = [2*(rawData.image.centerCol(1)-1), 2*(rawData.image.centerLin(1)-1), rawData.image.NPar]; % image size
param.mxCoef        = rawData.hdr.Meas.aflMaxwellCoefficients;                      % Maxwell coefficients
param.patPos        = rawData.hdr.Meas.PatientPosition;                             % Patient position
param.TE            = rawData.hdr.MeasYaps.alTE;                                    % Echo Time (TE)
param.TRes          = rawData.hdr.MeasYaps.alTR{1}/1000;                            % Temporal resolution
param.TR            = rawData.hdr.Meas.lEchoSpacing;                                % Echo spacing / TR
param.FA            = rawData.hdr.Dicom.adFlipAngleDegree;                          % Flip angle
param.cCol          = mean(rawData.image.centerCol);                                % central column; used to fix the center along FE
param.acqStartTime  = rawData.image.timestamp(1)*2.5;                               % Acquistion start time

if isfield(rawData.hdr.Config, 'DwellTime')
    param.BW = 1/(rawData.hdr.Config.DwellTime * 1e-9);                             % Note, the param.BW = BW_scanner * Npixels; where Npixels is the number of pixels (with oversampling) in FE
elseif isfield(rawData.hdr.Meas, 'alDwellTime')
    param.BW = 1/(rawData.hdr.Meas.alDwellTime(1) * 1e-9);                          %??
else
    error('BW information cannot be located');
end

%% Flow (phase-contrast) module
% ????????????????????????????????????????? Uncomment this for flow
% Perhaps there is a way to check this automatically?
if flow
tmp = rawData.hdr.MeasYaps.sAngio.sFlowArray.lSize;
for i=1:tmp
    rawData.hdr.MeasYaps.sAngio.sFlowArray.asElm{i};
    param.venc(i) = rawData.hdr.MeasYaps.sAngio.sFlowArray.asElm{i}.nVelocity;      % Venc values
    param.nDir(i) = rawData.hdr.MeasYaps.sAngio.sFlowArray.asElm{i}.nDir;           % Direction of encoding
end
end
% ?????????????????????????????????????????
...
%% Adjust zero-padding in FE direction so that peak occurs in the center
...
% z3 = 2*rawData.image.centerLin(1)-1-2*(ceil(rawData.image.centerLin(1)/2)-rawData.image.centerLin(1)/2) - size(data,lnDim);
% if z3 > 0
...
cropPerc = fd(1)+fd(2);
disp(['Cropping FE dimension by ', num2str(100*cropPerc), '%']);
data = single(data);
...
if ~isempty(ref)
    I = fftshift(ifft(ifftshift(ref,1),[],1),1)* sqrt(size(ref,1));
    I(1:round(size(ref,1)*fd(1)),:,:,:,:,:,:,:,:,:)=[]; % I am assuming, ndims(data) <= 10 %??
    I(end-round(size(ref,1)*fd(2))+1:end,:,:,:,:,:,:,:,:,:)=[]; %??
    ref = fftshift(fft(ifftshift(I,1),[],1),1)/sqrt(size(I,1));% / sqrt(N(1)/(N(1)-z1-z2));
end
...
%     samp = padarray(samp,2*round(z1*(1-fd(1)-fd(2))/2),0,'pre');
    data(1:ceil(z1*cropPerc),:,:,:,:,:,:,:,:,:) = 0;
    samp(1:ceil(z1*cropPerc),:,:,:,:,:,:,:,:,:) = 0;
elseif z2>0
%     data = padarray(data,2*round(z2*(1-fd(1)-fd(2))/2),0,'post');
%     samp = padarray(samp,2*round(z2*(1-fd(1)-fd(2))),0,'post');
    data(end:-1:end-ceil(z2*cropPerc)+1,:,:,:,:,:,:,:,:,:) = 0;
    samp(end:-1:end-ceil(z2*cropPerc)+1,:,:,:,:,:,:,:,:,:) = 0;
end
