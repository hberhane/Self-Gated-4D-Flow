...
if wavelet_only
    ...
    % Number of rows in the analysis operator
    anaSizeM = W.M;
else
    % tPCA and Wavelets stacked
    trans1 = tPCA(size(obj.data.sampB));
    w_prop.wname = 'db1';
    w_prop.level = 1;
    trans2 = ndDWTLinTrans(w_prop,size(obj.data.sampB),'compute',obj.options.ReVEALOpts.compute,...
             'pres_l2_norm',0,'precision',obj.options.ReVEALOpts.precision);
    sparseTrans = sparseTransConcat({trans1,trans2},[1,8],size(obj.data.sampB));
    
    % Set lambda
%     lambda1 = 10;
    lambda1 = 5*lambda0;
    lambda = lambda1*ones(numel(obj.data.sampB),1);
    lambda_w = setLambda(size(obj.data.sampB),lambda0);
    lambda = [lambda;lambda_w(:)];
    
    % Concatonate All The Linear Transform Operator Together
    Op_b = LinTransConcat({pMRI_b;sparseTrans},[1,1],obj.options.ReVEALOpts.precision,obj.options.ReVEALOpts.compute);
    Op_v = LinTransConcat({pMRI_v;sparseTrans},[1,1],obj.options.ReVEALOpts.precision,obj.options.ReVEALOpts.compute);

    % Number of rows in the analysis operator
    anaSizeM = sparseTrans.M;
%     
%         % tPCA and Wavelets stacked
%     trans1 = tFFT(size(obj.data.sampB));
%     w_prop.wname = 'db1';
%     w_prop.level = 1;
%     trans2 = ndDWTLinTrans(w_prop,size(obj.data.sampB),'compute',obj.options.ReVEALOpts.compute,...
%              'pres_l2_norm',0,'precision',obj.options.ReVEALOpts.precision);
...
% x = zeros(size(obj.data.x0));
% x(1) = 1;
% a = sparseTrans.mult(x(:));
% x2 = zeros([size(obj.data.x0),9]);
% x2(1) = 1;
% ah = sparseTrans.multTr(x2(:));
% a(1)
% ah(1)
% 
% 
% x = zeros(size(obj.data.x0));
% x(1) = 1;
% asq = sparseTrans.multSq(x(:));
% 
% x2 = zeros([size(obj.data.x0),9]);
% x2(1) = 1;
% asqh = sparseTrans.multSqTr(x2(:));
% asq(1)
% asqh(1)
...
EstimOut_b = EstimOutConcat({MeasEstimOut_b;AnaEstimOut1},[pMRI_b.M,anaSizeM],...
    obj.options.ReVEALOpts.precision,obj.options.ReVEALOpts.compute);
EstimOut_v = EstimOutConcat({MeasEstimOut_v;AnaEstimOut1},[pMRI_v.M,anaSizeM],...
    obj.options.ReVEALOpts.precision,obj.options.ReVEALOpts.compute);
...
reveal_start = tic;
if L1
    xhatGAMP_b = gampEstLowMem(EstimIn_b,EstimOut_b,Op_b,GAMPoptB);
    xhatGAMP_v = gampEstLowMem(EstimIn_v,EstimOut_v,Op_v,GAMPoptV);
else
...
% % Estimate relative lamba
% zhat = xhatGAMP_b.zhat(size(Op_b.lta{1})+1:end);
% zhat = reshape(zhat,[size(obj.data.sampB),9]);
% for ii = 1:size(zhat,4)
%     tmp = zhat(:,:,:,ii);
...
% wave = sparseTrans.mult(xhatGAMP_b.xhat);
% wave = reshape(wave,[size(obj.data.sampB),9]);
% for ind = 1:9
%     tmp = wave(:,:,:,ind);
%     lam(ind) = 1/mean(abs(tmp(:)));
% end
%==========================================================================
% Main Loop
for ind = 2:ReVEALOpts.nit
    ...
    GAMPoptB.nit = 10;
    GAMPoptV.nit = 10;
%     GAMPoptB.adaptStep = 0;
%     GAMPoptB.stepIncr = 1;
%     GAMPoptB.stepDecr = 1;
%     GAMPoptB.step = 0.15;
%     
%     GAMPoptV.adaptStep = 0;
%     GAMPoptV.stepIncr = 1;
%     GAMPoptV.stepDecr = 1;
%     GAMPoptV.step = 0.15;
%     
