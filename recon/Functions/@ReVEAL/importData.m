...
if exist(fileName)
    ...
    obj.data.scanParam = data.scanParam; %RA
elseif exist([fileName(1:end-4),'B.mat'])
    ...
    if exist([fileName(1:end-8),'scanParam.mat'])
        obj.data.scanParam = importdata([fileName(1:end-8),'scanParam.mat']);
    end
    
    if exist([fileName(1:end-8),'weightsB.mat'])
        obj.data.weightsB = importdata([fileName(1:end-8),'weightsB.mat']);
        obj.data.weightsX = importdata([fileName(1:end-8),'weightsX.mat']);
        obj.data.weightsY = importdata([fileName(1:end-8),'weightsY.mat']);
        obj.data.weightsZ = importdata([fileName(1:end-8),'weightsZ.mat']);
    end
%     obj.data.scanParam = importdata([fileName(1:end-8),'scanParam.mat']); %RA
...
%                     obj.options.ReVEALOpts.wvar = estimNoiseVar(obj.data.noiseChannel,obj.data.sampB)
                    k = squeeze(obj.data.noiseChannel).*obj.data.sampB;
                    k = k(1:end,end-10:end,1:end);
                    k = k(abs(k)>0);
                    obj.options.ReVEALOpts.wvar=var(k)/1.2; %RA added 1.2
                else
...
elseif ndims(obj.data.Yb) == 5
    ...
    if size(obj.data.Yb,4) > obj.options.ReVEALOpts.num_coils
        ...
        [kdata,noise_ch] = coilCombine(kdata,obj.options.ReVEALOpts.num_coils,'3dt');
        obj.data.Yb = kdata(:,:,:,:,:,1);
        obj.data.Yx = kdata(:,:,:,:,:,2);  
        obj.data.Yy = kdata(:,:,:,:,:,3); 
        obj.data.Yz = kdata(:,:,:,:,:,4);
    ...
    if ~isempty(obj.data.noiseChannel) && isempty(obj.options.ReVEALOpts.wvar)
        obj.options.ReVEALOpts.wvar = var(obj.data.noiseChannel(:));
    end
...
%     obj.data.x0 = obj.data.x0/scale;
    ...
    obj.data.Yy = obj.data.Yy/scale;
    obj.data.Yz = obj.data.Yz/scale;
    obj.options.ReVEALOpts.wvar;
    obj.options.ReVEALOpts.wvar = obj.options.ReVEALOpts.wvar/scale^2;
    obj.options.ReVEALOpts.wvar;
    % Set sigmaSq if none is provided
    if isempty(obj.options.ReVEALOpts.sigmaSq)
        obj.options.ReVEALOpts.sigmaSq = obj.options.ReVEALOpts.wvar/sqrt(size(obj.data.Yb,4));
    end
...
fprintf(sprintf('Scaled Noise Variance = %s\n',num2str(obj.options.ReVEALOpts.wvar)));
