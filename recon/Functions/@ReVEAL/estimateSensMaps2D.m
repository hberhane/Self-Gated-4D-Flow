...
%     time_av = time_av_b;
    ...
    warning('using Walsh Maps')
    
    %% sensitivity estimation parameters
%     p.mthd   = 1; %'1' espirit, '2' Walsh
%     p.fil    = [6,6,1];  % size of kernal for eSPIRiT or size of filter for 'Walsh'; use [6,6,6] for eSPIRiT and [3,1,1] for Walsh
%     p.ACSsz  = [size(obj.data.sampB,1),size(obj.data.sampB,2),1]; % size of the k-space block used for eSPIRiT
%     p.eSRT   = 0.95;
...
%     [walsh_maps,x0] = coilSen(obj.data.Yb,obj.data.sampB,p);
